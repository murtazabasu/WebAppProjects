...
export default function Login() {
  ...
  async function handleSubmit() {
    ...
    try {
      if (isRegister) {
        console.log('Singing up a new user...')
  ...
  return (
    <div className='flex flex-col flex-1 justify-center items-center gap-4'>
      ...
      <p className='text-center'>{ isRegister ? 'Already have an account?' : 'Don\'t have an account'} <button onClick={() => setIsRegister(!isRegister)} 
